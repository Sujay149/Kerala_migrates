    ...
    try {
      ...
      console.log('📊 User data summary:', 
        userDataResult.userData ? {
          profile: !!userDataResult.userData.profile,
          submissions: userDataResult.userData.submissions?.length || 0,
          healthRecords: userDataResult.userData.healthRecords?.length || 0,
          medications: userDataResult.userData.medications?.length || 0,
          userEmail: userDataResult.userData.userEmail,
          userId: userDataResult.userData.userId
        } : 'No user data'
      );
